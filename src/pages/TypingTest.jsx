...
function getWordCharClass(wordIndex, charIndex, currentWordIndex, input, word, globalOffset) {
  ...
  if (wordIndex < currentWordIndex) {
    ...
    return typedWord === word ? 'text-emerald-400' : 'text-red-400'
  }

  if (wordIndex > currentWordIndex) return 'text-zinc-500'

  if (absoluteIndex < input.length) {
    return input[absoluteIndex] === word[charIndex] ? 'text-emerald-300' : 'text-red-400'
  }
  ...
  if (finished) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-16">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-8 text-center shadow-xl shadow-black/20">
          <p className="text-sm font-medium uppercase tracking-wider text-emerald-400">Time&apos;s up</p>
          <h1 className="mt-2 text-3xl font-semibold text-white">Results</h1>

          <dl className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
              <dt className="text-xs uppercase tracking-wider text-zinc-500">WPM</dt>
              <dd className="mt-1 text-3xl font-bold text-white">{finalWpm}</dd>
            </div>
            <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
              <dt className="text-xs uppercase tracking-wider text-zinc-500">Accuracy</dt>
              <dd className="mt-1 text-3xl font-bold text-white">{finalAccuracy}%</dd>
            </div>
            <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
              <dt className="text-xs uppercase tracking-wider text-zinc-500">Peeks</dt>
              <dd className="mt-1 text-3xl font-bold text-white">{peeks}</dd>
            </div>
            <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-4">
              <dt className="text-xs uppercase tracking-wider text-zinc-500">Eye discipline</dt>
              <dd className="mt-1 text-3xl font-bold text-white">{eyeDiscipline}%</dd>
            </div>
          </dl>

          {saveStatus && (
            <p className="mt-6 text-sm text-zinc-400">{saveStatus}</p>
          )}
    ...
    <>
      {isLookingDown && (
        <>
          ...
          <div
            role="alert"
            className="pointer-events-none fixed left-1/2 top-20 z-50 -translate-x-1/2 rounded-lg border border-red-500/40 bg-red-950/90 px-4 py-2 text-sm font-semibold text-red-200 shadow-lg shadow-red-950/50"
          >
            Eyes up!
          </div>
        </>
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-white">Typing Test</h1>
        ...
        <div className="flex gap-6 text-sm">
          <div>
            ...
            <p className="text-xl font-semibold tabular-nums text-white">{started ? liveWpm : '—'}</p>
          </div>
          <div>
      ...
      <div
        ...
        className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-8 outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30"
      >
        <p className="text-2xl leading-relaxed">
          {words.map((word, wordIndex) => {
            const wordStart = globalOffset
            globalOffset += word.length + (wordIndex < words.length - 1 ? 1 : 0)
                ...
                <span
                  className={
                    wordIndex === currentWordIndex
                      ? 'rounded bg-yellow-400/25 px-0.5'
                      : undefined
                  }
                >
