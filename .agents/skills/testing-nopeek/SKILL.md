...
- **Historic landmine (fixed):** navigating away from `/calibrate` used to blank the entire app because
  the effect cleanup called `window.webgazer.end()`, which throws when WebGazer never started. The
  cleanup is now wrapped in try/catch and an `ErrorBoundary` wraps `<Routes>`, so you should now see
  `WebGazer shutdown failed:` / `WebGazer preview unavailable:` **warnings** and a normally rendered
  destination page. If a page ever blanks after leaving `/calibrate` again, that regression is back.
- Expect these benign console entries on any `/calibrate` visit without a camera:
  `loaded stored data into regression model`, `No stream`, and the two warnings above.

## ErrorBoundary (src/components/ErrorBoundary.jsx)
- Wraps `<Routes>` as `<ErrorBoundary key={location.pathname}>`, so a latched error is cleared simply by
  navigating to another route. Fallback = h1 "Something went wrong" + `error.message` + a "Try again"
  button.
- To see the fallback, temporarily add `throw new Error('Test-only forced render error')` at the top of
  a page component and revert afterwards (`git status` to confirm nothing is left changed).
- Gotcha: reverting that edit triggers a Vite HMR remount that heals the page automatically, so you
  cannot easily observe the "Try again" button restoring a healthy page — clicking it while the throw is
  still in place just re-renders into the same error. Report that sub-check as inconclusive rather than
  guessing.
